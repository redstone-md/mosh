...
const VPN_NAME_NEEDLES: &[&str] = &[
    ...
    "tailscale", "clearvpn", "expressvpn", "surfshark",
    "vethernet", "hyper-v", "virtual", "vmware",
    "vbox", "loopback", "pseudo", "vpn",
];
...
#[cfg(windows)]
mod windows_impl {
    use super::{name_looks_virtual, NetworkInterfaceInfo};
    use std::ffi::OsString;
    use std::os::windows::ffi::OsStringExt;
    use windows_sys::Win32::NetworkManagement::IpHelper::{
        FreeMibTable, GetAdaptersAddresses, GetIpForwardTable2,
        GAA_FLAG_INCLUDE_PREFIX, GAA_FLAG_SKIP_ANYCAST, GAA_FLAG_SKIP_DNS_SERVER,
        GAA_FLAG_SKIP_MULTICAST, IP_ADAPTER_ADDRESSES_LH, MIB_IPFORWARD_TABLE2,
    };
    ...
    pub fn enumerate() -> Result<Vec<NetworkInterfaceInfo>, String> {
        ...
        for adapter in &raw {
            ...
            if !is_virtual && !is_physical && name_looks_virtual(&name) {
                is_virtual = true;
            }
            if !is_virtual && is_physical && name_looks_virtual(&name) {
    ...
    unsafe fn collect_first_ipv4(adapter: &IP_ADAPTER_ADDRESSES_LH) -> Option<String> {
        ...
        while !unicast.is_null() {
            ...
            if !socket_addr.is_null() {
                ...
                if family == AF_INET {
                    let raw = socket_addr as *const windows_sys::Win32::Networking::WinSock::SOCKADDR_IN;
