    ...
    fn dm_offers_are_roster_gated_targeted_and_accept_once() {
        ...
        let roster = signed_roster(
            1,
            &[(own_peer.as_str(), "alice", "admin"), (member_peer.as_str(), "bob", "member")],
        );
        ...
        runtime.ingest_for_test(&org, &signed_wire(&member_key, mesh, &offer("o1", &own_peer)));
        runtime.ingest_for_test(&org, &signed_wire(&member_key, mesh, &offer("o1", &own_peer)));
        // From a stranger (envelope valid, not in roster): dropped.
        runtime.ingest_for_test(&org, &signed_wire(&stranger_key, mesh, &offer("o2", &own_peer)));
        // Aimed at someone else: ignored.
        runtime.ingest_for_test(&org, &signed_wire(&member_key, mesh, &offer("o3", &member_peer)));
        ...
        let v2 = signed_roster(2, &[(own_peer.as_str(), "alice", "admin")]);
        runtime.ingest_for_test(&org, &roster_wire(&v2));
        runtime.ingest_for_test(&org, &signed_wire(&member_key, mesh, &offer("o4", &own_peer)));
        let snap = runtime.poll(&org).unwrap();
        assert!(snap.dm_offers.is_empty());
        assert_eq!(snap.dm_links.len(), 1);
