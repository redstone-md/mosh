...
**Tech Stack:** Go (moss, MossSpore), Rust/Tauri (mosh), cgo FFI (`Moss_*` exports), systemd deploy on <relay-host>.
...
- [ ] **Step 3: Deploy** to <relay-host> (current prod runs pre-substrate `v0.3.2+moss-d9760fb` — replace): cross-compile `GOOS=linux GOARCH=amd64`, scp binary, restart `systemd mossspore`, verify `curl 127.0.0.1:9800/health` + `/version` shows the new build, watch promotion (needs first peer after restart; same-IP colocation kills announces — the box must run exactly one spore).
...
- [ ] **Step 1:** `RELAY_BOOTSTRAP_SPORES = &["<relay-host>:4001"]` (existing well-formedness test covers it). Commit `feat(dm): seed relay bootstrap with the mossspore relay`.
