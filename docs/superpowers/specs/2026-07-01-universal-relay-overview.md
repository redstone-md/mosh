...
|---|---|---|---|---|
| **S1** | moss: relay-by-peer-id FFI | `moss` | — | Audit found Mesh TURN by peer-id (`OpenRelaySessionAny`/`RelaySendTo`, E2E via SuperNode) already exists, and a mutation test proved the peer's Noise static arrives via signed peer-announce flooding (no seeding needed). S1 = two FFI exports only (`Moss_RelaySendTo`/`Moss_SetRelayCallback`). Relay-mesh membership is a second moss node run by mosh, not a core change. |
