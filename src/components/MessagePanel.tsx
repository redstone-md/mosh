    ...
    <section className="flex-1 flex flex-col min-h-0 bg-background relative">
      {activeSearchMessageId && activeSearchPreview ? (
        <div className="border-b border-border bg-[var(--panel-strong)] px-6 py-2">
          <div className="mx-auto max-w-4xl text-xs text-[var(--muted-foreground)]">{activeSearchPreview}</div>
        </div>
      ) : null}
              ...
              >
                <div
                  className={cn(
                    ...
                    message.emphasis === 'system'
                      ? 'bg-primary/20 text-primary border border-primary/20'
                      : 'bg-secondary text-foreground/80 border border-border/20'
                  )}
                  aria-hidden="true"
                >
                ...
                <div className="flex-1 min-w-0 space-y-1">
                  <div className="flex items-center gap-3">
                    <span className="font-bold text-[15px] tracking-tight text-foreground/90">{message.author}</span>
                  ...
                  {canManageMessage(message, currentUser) ? (
                    <>
                      ...
                      <button
                        ...
                        className="p-1.5 text-foreground/50 hover:text-foreground hover:bg-muted rounded-md transition-colors"
                        title={
                          message.overlayState === 'hidden' ? copy.messages.restoreMessage : copy.messages.hideMessage
                        }
                      >
                        <Trash2 size={14} />
                      </button>
      ...
      <div className="p-6 pt-0">
        <div className="max-w-4xl mx-auto relative">
          ...
          {errorNote ? <p className="mt-2 text-xs text-red-400 font-medium px-4">{errorNote}</p> : null}
        </div>
      </div>
      <MessageEditDialog
