import { CircleDot, Cloud, Network, Power, PowerOff, TriangleAlert, Waypoints } from 'lucide-react'
    ...
    <section className="bg-primary/5 border-b border-primary/10 px-4 py-2 flex items-center justify-between gap-4 shrink-0 text-sm">
      <div className="flex items-center gap-6 min-w-0">
        <div className="flex items-center gap-2 min-w-0">
          <div
            className={`w-2.5 h-2.5 rounded-full ${isOnline ? 'bg-primary shadow-[0_0_0_4px_rgba(90,198,136,0.2)]' : 'bg-foreground/30'}`}
          />
        ...
        <div className="hidden md:flex gap-2 text-xs font-mono">
          <span className="px-2 py-0.5 rounded-full bg-background border border-border/50 text-foreground/70">
            {natHint}
          </span>
          <span className="px-2 py-0.5 rounded-full bg-background border border-border/50 text-foreground/70">
            {route}
  ...
  return (
    <span
      className={`flex h-7 w-7 items-center justify-center rounded border text-[11px] ${
        active ? 'border-primary/30 bg-primary/10 text-primary' : 'border-border/80 bg-[var(--chat)] text-foreground/55'
      } ${className ?? ''}`}
      title={title}
    >
      <Icon size={13} />
